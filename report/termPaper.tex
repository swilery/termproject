...
\textit{Definition:} A \textit{context-free grammar}, $CFG$, is a tuple $(N, T, P, S)$, where, \cite{bilardi-pingali}:
\begin{itemize}
...
\begin{itemize}
\item[$\diamond$] $(M\longrightarrow\bullet{q_1q_2\dots{q_r}}), (M\longrightarrow{q_1\bullet{q_2...q_r}}), \dots, (M\longrightarrow{q_1q_2...q_r\bullet}) \in V(CFG)$, where the first node is called an \textit{entry node} and the last node is called an \textit{exit node},
